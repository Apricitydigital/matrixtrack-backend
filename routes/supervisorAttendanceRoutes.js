...
/**
 * @route GET /api/supervisor-attendance
 * @desc Fetch supervisor presence statistics and daily history details
 * @access Protected (Requires Authentication and City Scope check)
 */
router.get("/", authenticate, attachCityScope, requireCityScope(), async (req, res) => {
  ...
  try {
    ...
    // Validate date range limit to prevent database overload
    ...
    if (diffDays > 45) {
      return res.status(400).json({ error: "Date range cannot exceed 45 days" });
    }
    ...
    let wardIdsFilter = null;
    if (wardId && wardId !== "all" && wardId !== "undefined") {
      wardIdsFilter = String(wardId).split(",").map(Number).filter(n => !isNaN(n));
      if (wardIdsFilter.length === 0) {
        wardIdsFilter = null;
      }
    }
    ...
    const query = `
      ...
      supervisors AS (
        SELECT 
          ...
          STRING_AGG(DISTINCT s.sector_name, ', ') AS wards,
          ...
          COALESCE((
            ...
            WHERE e.ward_id IN (
              ...
              WHERE sw2.supervisor_id = u.user_id
            )
        ...
        WHERE u.role = 'supervisor'
          ...
          AND ($7::int[] IS NULL OR w.ward_id = ANY($7::int[]))
      ...
      GROUP BY 
        ...
        s.city_id, s.city_name, s.zones, s.wards, s.kothis,
        s.total_employees
    ...
    console.error("Error in supervisor attendance endpoint:", error);
