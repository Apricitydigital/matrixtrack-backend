...
const normalizeLivenessFrames = (rawFrames, primaryFrame = null) => {
  const parsed = parseLivenessFrames(rawFrames);
  const normalized = [];

  if (typeof primaryFrame === 'string' && primaryFrame.trim().length > 0) {
    normalized.push(primaryFrame.trim());
  }

  for (const frame of parsed) {
    const value = frame.trim();
    if (!value) continue;
    if (normalized.length === 0 || normalized[normalized.length - 1] !== value) {
...
const getChallengeFrameForFaceMatch = (rawFrames, primaryFrame = null) => {
  const normalized = normalizeLivenessFrames(rawFrames, primaryFrame);
...
const runLivenessPrecheck = async (selfieBase64, options = {}) => {
  ...
  const parsedFrames = normalizeLivenessFrames(options?.livenessFrames, selfieBase64);
  ...
  try {
    ...
    if (!matchResult.isMatch) {
      ...
      return res.status(403).json({
        success: false,
        code: 'FACE_MISMATCH',
        message: 'Face not matched with this ID. Please use correct ID.',
    ...
    const challengeFrame = getChallengeFrameForFaceMatch(liveness_frames, selfie_base64);
    if (challengeFrame) {
...
const punchOut = async (req, res) => {
  ...
  try {
    ...
    // 3. Liveness pre-check after identity verification to avoid confusing mismatch as liveness failure
    const liveness = await runLivenessPrecheck(selfie_base64, {
      livenessFrames: liveness_frames,
      livenessChallenge: liveness_challenge,
    });
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (record) {
        ...
        if (record.punch_in && record.punch_out) {
          // Fully completed session ΓÇö use stored hours_worked
          ...
          // Punched in but no punch-out ΓÇö counts as present, hours shown as '-'
          status = 'present';
