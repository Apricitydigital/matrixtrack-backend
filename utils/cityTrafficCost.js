...
const ensureCityTrafficSchema = async () => {
  ...
  await pool.query(`
    ALTER TABLE city_daily_traffic_cost
      ...
      CHECK (source IN ('group_attendance', 'individual_attendance', 'professional_punch_in', 'professional_punch_out', 'professional_access_request', 'face_enrollment'));
  `);
...
const flushTrafficBuffer = async () => {
  ...
  try {
    ...
    logger.error(`[CityTrafficCost] Failed schema check/connection on buffer flush: ${schemaError.message}`);
  }
};
