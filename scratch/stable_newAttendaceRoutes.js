...
// Legacy sync check ΓÇö used only as last-resort guard on the attendance object itself
...
/**
 * ≡ƒöÆ SESSION-AWARE PUNCH VALIDATION (Night-Shift + Re-Punch-In Guard)
 *
 ...
 *   - 11 PM punch-in ΓåÆ attendance_date = Day1 (formatDate uses NIGHT_SHIFT_ROLLOVER_HOUR)
 *   - 4 AM punch-out ΓåÆ finds the open session from Day1, closes it correctly
...
// ΓöÇΓöÇΓöÇ COST OPTIMIZATION: Individual punch fallback loop ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// When SearchFacesByImage returns no match, fallbackMatchByCompare runs a
// CompareFaces call for EVERY employee in the ward ΓÇö extremely expensive at scale.
...
// ΓöÇΓöÇΓöÇ COST OPTIMIZATION: 60-second punch dedup cache ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// Prevents double-billing when supervisor retries on network timeout.
// key: `${empId}:${punchType}:${date}` ΓåÆ timestamp of last successful punch.
...
// ≡ƒöÆ Check if a CLOSED session already exists for the given attendance_date
...
async function getOrCreateAttendanceRecord(emp_id, date, options = {}) {
  ...
  // ΓÜí PERF FIX: Single query fetches ward_id + display fields together (was 2 separate queries)
  ...
  // ΓÜí PERF FIX: Reuse empMeta fetched above ΓÇö no additional JOIN query needed
...
async function processPunch(
  ...
  const {
    ...
    uploadContext: preloadedContext = null, // ΓÜí PERF FIX: accept pre-fetched context
  } = options;
  ...
  if (imageFile) {
    // ΓÜí PERF FIX: use preloaded context if available, skips 4-table JOIN DB call
  ...
  const result = await pool.query(updateQuery, [
    ...
    imageUrl,
    normalizeId(userId) ?? null, // ΓÜí PERF FIX: userId already auth'd by JWT, no DB call needed
    attendanceId,
  ]);
...
// ΓÜí PERF FIX: resolvePunchActor removed ΓÇö userId from JWT is already authenticated.
...
async function loadFaceBuffer(faceEmbedding, employeeId = null, empCode = null) {
  ...
  // ≡ƒ¢í∩╕Å Self-Healing Prefix Scan if the direct key fails
  if (employeeId) {
...
async function fallbackMatchByCompare(
  ...
  if (best) {
    // ≡ƒÆí AUTO-HEAL: If this employee's face is not in the Rekognition collection,
...
async function ensureFaceMatch(employeeId, attendanceKey, threshold) {
  ...
  // ≡ƒ¢í∩╕Å MULTI-BUCKET RESOLUTION (Trying all possible buckets from environment)
  const bucketsToTry = [
    AWS_S3_BUCKET,
  ...
  // ΓÜí PERF FIX: S3 prefix scan wrapped in 1.5s timeout.
